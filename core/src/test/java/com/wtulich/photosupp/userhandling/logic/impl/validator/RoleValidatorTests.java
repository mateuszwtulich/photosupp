...
public class RoleValidatorTests {
    ...
    @Test
    @DisplayName("Test verifyIfRoleAlreadyExists2 Success")
    void verifyIfRoleAlreadyExistsSuccess2() {
        //Arrange
        when(permissionDao.findAllById(permissionsIds)).thenReturn(permissionEntities);
        when(roleDao.existsByName(roleEntity.getName())).thenReturn(false);
        when(roleDao.existsByPermissionsIn(permissionEntities)).thenReturn(true);
    ...
    void verifyIfRoleAlreadyExistsFailure() {
        ...
        //Act Assert
        Assertions.assertDoesNotThrow(() ->  roleValidator.verifyIfRoleAlreadyExists(roleTo));
    }
}
