...
public class OrderRestServiceTest {
    ...
    private static String MEDIA_CONTENT_URL = "/order/v1/order";
    ...
    private static String CLOSE_ORDER = "/order/v1/order/{id}/close";
    ...
    void setUp() {
        ...
        RoleEto roleEto2 = new RoleEto(1L, "USER", "Standard user with no special permissions", permissionEtoList);
        AccountEto accountEto2 = new AccountEto(1L, "user2", "passw0rd", "user2@test.com", true);
        UserEto userEto2 = new UserEto(1L, "NAME2", "SURNAME2", accountEto2, roleEto2);

        mediaContentEto = new MediaContentEto(1L, MediaType.IMAGE, "https://sample.com/jpg1", 1L);
        commentEto = new CommentEto(1L, "Perfect, thanks!", 1L, userEto,
                DateTimeFormatter.ofPattern( "yyyy-MM-dd" ).format(LocalDate.now()));
        OrderEto orderEto = new OrderEto(1L, "INVIU_00001", userEto, userEto2, OrderStatus.IN_PROGRESS, null, 1000D,
                DateTimeFormatter.ofPattern( "yyyy-MM-dd" ).format(LocalDate.now()));
    }
...
//        when(orderHandling.findAllMediaContentByOrderId(orderEto.getId())).thenReturn(Optional.of(mediaContentList));
    ...
    void testGetAllMediaContentByOrderIdNotFound() throws Exception {
        ...
        ArrayList<MediaContentEto> mediaContentList = new ArrayList<>();
        mediaContentList.add(mediaContentEto);
//        doThrow(EntityDoesNotExistException.class).when(orderHandling).findAllMediaContentByOrderId(indicatorEto.getId());
...
//        when(orderHandling.findAllCommentsByOrderId(orderEto.getId())).thenReturn(Optional.of(comments));
    ...
    void testGetAllCommentsByOrderIdNotFound() throws Exception {
        ...
        ArrayList<MediaContentEto> mediaContentList = new ArrayList<>();
        mediaContentList.add(mediaContentEto);
//        doThrow(EntityDoesNotExistException.class).when(orderHandling).findAllCommentsByOrderId(indicatorEto.getId());
...
//        when(orderHandling.updateComment(commentTo, commentEto.getId())).thenReturn(Optional.empty());
    ...
    @Test
    @DisplayName("PUT /order/v1/order/{id}/close - OK")
    void testCloseOrderOk() throws Exception {
        //Arrange
//        when(orderHandling.closeOrder(orderEto.getId())).thenReturn(orderEto);

        //Act
        MvcResult result = mockMvc.perform(put(CLOSE_ORDER, orderEto.getId()))
    ...
    @Test
    @DisplayName("PUT /order/v1/order/{id}/close - Not Found")
    void testCloseOrderNotFound() throws Exception {
        //Arrange
//        when(orderHandling.closeOrder(orderEto.getId())).thenThrow(EntityDoesNotExistException.class);

        //Act
        mockMvc.perform(put(CLOSE_ORDER, orderEto.getId()))
    ...
    @Test
    @DisplayName("PUT /order/v1/order/{id}/close - Unprocessable Entity")
    void testCloseOrderUnprocessableEntity() throws Exception {
        //Arrange
//        when(orderHandling.closeOrder(orderEto.getId())).thenThrow(OrderStatusInappropriateException.class);
    ...
    void testAcceptOrderOk() throws Exception {
        //Arrange
//        when(orderHandling.acceptOrder(orderEto.getId())).thenReturn(orderEto);
...
//        when(orderHandling.acceptOrder(orderEto.getId())).thenThrow(EntityDoesNotExistException.class);
    ...
    void testAccpetOrderUnprocessableEntity() throws Exception {
        //Arrange
//        when(orderHandling.acceptOrder(orderEto.getId())).thenThrow(OrderStatusInappropriateException.class);
    ...
    void testVerificationOrderOk() throws Exception {
        //Arrange
//        when(orderHandling.sentToVerificationOrder(orderEto.getId())).thenReturn(orderEto);
    ...
    void testVerficationOrderNotFound() throws Exception {
        //Arrange
//        when(orderHandling.sentToVerificationOrder(orderEto.getId())).thenThrow(EntityDoesNotExistException.class);
    ...
    void testVerificationOrderUnprocessableEntity() throws Exception {
        //Arrange
//        when(orderHandling.sentToVerificationOrder(orderEto.getId())).thenThrow(OrderStatusInappropriateException.class);
